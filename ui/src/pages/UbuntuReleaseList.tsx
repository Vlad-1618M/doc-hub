...
import { fetchUbuntuReleases } from '../api/recordsApi'
import { isRecent } from '../lib/recentGlow'
import type { UbuntuRelease } from '../types'

const PAGE_SIZE = 25

export function UbuntuReleaseList() {
  const { token } = useAuth()
  const { theme } = useTheme()
  const [search, setSearch] = useState('')
  const [searchInput, setSearchInput] = useState('')
  const [page, setPage] = useState(0)
  const [items, setItems] = useState<(UbuntuRelease & { _id: string })[]>([])
  ...
  const exportJson = () => {
    ...
    a.href = url
    a.download = `ubuntu-releases-page${page + 1}.json`
    a.click()
  ...
  const exportPdf = () => {
    const doc = new jsPDF()
    doc.setFontSize(14)
    doc.text('Ubuntu Releases', 20, 20)
    doc.setFontSize(10)
    let y = 30
    for (const item of items) {
      if (y > 270) { doc.addPage(); y = 20 }
      doc.text(`${item.version} · ${item.codename} · ${item.release_date} → ${item.eol_date}`, 20, y)
      y += 7
    }
    doc.save('ubuntu-releases-list.pdf')
    setExportOpen(false)
  }
  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
        <div>
          <h1 className="font-display text-2xl font-semibold text-slate-900">Ubuntu Releases</h1>
          <p className="mt-1 text-slate-500">Linux distribution release metadata</p>
        </div>
        <div className="flex shrink-0 gap-2">
          ...
          <Link to="new" className="btn-primary">
            <span className="mr-2">+</span> Add Release
          </Link>
        </div>
      </div>
      ...
      <div className="card overflow-hidden">
        <form onSubmit={handleSearch} className="flex flex-wrap items-center gap-4 border-b border-slate-200 p-4">
          <input
            type="search"
            placeholder="Search by version, codename..."
            value={searchInput}
          ...
          <table className="w-full">
            <thead>
              <tr className="border-b border-slate-200 bg-slate-50/80">
                <th className="px-6 py-3 text-left text-xs font-medium uppercase tracking-wider text-slate-500">Version</th>
                <th className="px-6 py-3 text-left text-xs font-medium uppercase tracking-wider text-slate-500">Codename</th>
                <th className="px-6 py-3 text-left text-xs font-medium uppercase tracking-wider text-slate-500">Release / EOL</th>
            ...
            <tbody className="divide-y divide-slate-100">
              {items.map((item) => {
                ...
                >
                  ...
                  <td className="px-6 py-4 text-sm text-slate-600">{item.codename}</td>
                  <td className="px-6 py-4 text-sm text-slate-600">
                    {item.release_date} → {item.eol_date}
                  </td>
