...
import { fetchPythonReleases } from '../api/recordsApi'
import { isRecent } from '../lib/recentGlow'
import type { PythonRelease } from '../types'

const PAGE_SIZE = 25

export function PythonReleaseList() {
  const { token } = useAuth()
  const { theme } = useTheme()
  const [search, setSearch] = useState('')
  const [searchInput, setSearchInput] = useState('')
  const [page, setPage] = useState(0)
  const [items, setItems] = useState<(PythonRelease & { _id: string })[]>([])
  ...
  const exportJson = () => {
    ...
    a.href = url
    a.download = `python-releases-page${page + 1}.json`
    a.click()
  ...
  const exportPdf = () => {
    const doc = new jsPDF()
    doc.setFontSize(14)
    doc.text('Python Releases', 20, 20)
    doc.setFontSize(10)
    let y = 30
    for (const item of items) {
      if (y > 270) { doc.addPage(); y = 20 }
      doc.text(`Python ${item.version} · ${item.status} · ${item.release_date} → ${item.eol_date}`, 20, y)
      y += 7
    }
    doc.save('python-releases-list.pdf')
    setExportOpen(false)
  }
  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
        <div>
          <h1 className="font-display text-2xl font-semibold text-slate-900">Python Releases</h1>
          <p className="mt-1 text-slate-500">Python language version metadata</p>
        </div>
      ...
      <div className="card overflow-hidden">
        <form onSubmit={handleSearch} className="flex flex-wrap items-center gap-4 border-b border-slate-200 p-4">
          <input
            type="search"
            placeholder="Search by version, status..."
            value={searchInput}
