...
import { fetchUbuntuReleaseById, updateUbuntuRelease } from '../api/recordsApi'
import { RECORD_SCHEMAS } from '../config/recordSchemas'
import type { UbuntuRelease } from '../types'

const SCHEMA = RECORD_SCHEMAS.find((s) => s.id === 'ubuntu_releases')!

function exportJson(item: UbuntuRelease & { _id: string }) {
  const blob = new Blob([JSON.stringify(item, null, 2)], { type: 'application/json' })
  const url = URL.createObjectURL(blob)
  const a = document.createElement('a')
  a.href = url
  a.download = `ubuntu-${item.version}-${item.codename}.json`
  a.click()
  URL.revokeObjectURL(url)
}

function exportPdf(item: UbuntuRelease & { _id: string }) {
  const doc = new jsPDF()
  doc.setFontSize(18)
  doc.text(`${item.version} — ${item.codename}`, 20, 20)
  doc.setFontSize(12)
  doc.setTextColor(80, 80, 80)
  doc.text(item.support_type, 20, 30)
  ...
  doc.save(`ubuntu-${item.version}-${item.codename}.pdf`)
}

export function UbuntuReleaseDetail() {
  const { id } = useParams()
  const { token } = useAuth()
  const { theme } = useTheme()
  const [item, setItem] = useState<(UbuntuRelease & { _id: string }) | null | undefined>(undefined)
  ...
  const load = useCallback(async () => {
    if (!id) return
    try {
      const r = await fetchUbuntuReleaseById(token, id)
      setItem(r ?? null)
      if (r) {
        setValues(
          Object.fromEntries(SCHEMA.fields.map((f) => [f.key, String((r as unknown as Record<string, unknown>)[f.key] ?? '')]))
        )
      }
    } catch {
      setItem(null)
  ...
  const handleSave = async () => {
    ...
    setSaveError(null)
    try {
      await updateUbuntuRelease(token, id, values as unknown as Omit<UbuntuRelease, '_id'>)
      setItem({ ...item, ...values })
    } catch (e) {
  ...
  if (item === null) {
    return (
      <div className="rounded-lg bg-amber-50 px-6 py-8 text-center text-amber-800">
        Release not found. <Link to="/app/ubuntu-releases" className="font-medium underline">Back to list</Link>
      </div>
    )
  }
