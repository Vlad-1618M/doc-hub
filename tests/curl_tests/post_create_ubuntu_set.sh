...
BASE_URL="http://doc-hub-api:8000/ubuntu-releases"
...
DATA_DIR="${SCRIPT_DIR}/../data_sets/ubuntu_releases"
...
echo -e "\n$JOB ${white}Creating${off} Ubuntu releases from ${cyan}$DATA_DIR${off}:"
$decorator_init
...
for f in "$DATA_DIR"/*.json; do
    ...
    if id=$(echo "$RESP" | jq -r '.id // empty'); [[ -n "$id" ]]; then
        printf "${JOB} ${cyan}%-24s${gray} --> ${green}created${off} id: ${yellow}%s${off} | count: %d\n" "$name" "$id" "$count"
    else
...
echo -e "$JOB ${green}Done.${off} Created up to 5 Ubuntu releases."
