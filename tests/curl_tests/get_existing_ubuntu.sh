...
[[ "$n" -eq 0 ]] && { echo -e "$JOB ${gray}No Ubuntu releases in DB.${off}"; exit 0; }
count=1
echo "$JSON" | jq -c '.[]' 2>/dev/null | while read -r doc; do
    ver=$(echo "$doc" | jq -r '.version // "?"')
    codename=$(echo "$doc" | jq -r '.codename // "?"')
    id=$(echo "$doc" | jq -r '._id // "?"')
    printf "${JOB} ${cyan}%-8s %-16s${gray} --> id: ${yellow}%s${off} | %d\n" "$ver" "$codename" "$id" "$count"
    count=$((count + 1))
done
