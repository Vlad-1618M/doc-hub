...
_raw = (os.getenv("APP_LOG_LEVEL") or os.getenv("UVICORN_LOG_LEVEL") or "INFO").strip().upper()
DEFAULT_LOG_LEVEL = getattr(logging, _raw, logging.INFO)
...
def get_logger(name, log_file_path=LOG_FILE, level=None):
    if level is None:
